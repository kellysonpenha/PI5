...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  if (isset($_POST['cadastrar_aluno'])) {
    try {   
      ...
      echo '<script>$("#aluno").prepend("<div class=\"alert alert-success alert-dismissible\"><button type=\"button\" class=\"close\" data-dismiss=\"alert\" aria-hidden=\"true\">×</button><h4><i class=\"icon fa fa-check\"></i> Parabéns!</h4> Usuário cadastrado com sucesso!</div>");</script>';
    } 
  ...
  if (isset($_POST['cadastrar_funcionario'])) {
    try {   
      ...
      $pdo_conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION); 
      $pdo_conn = $pdo_conn->prepare("INSERT INTO UsuarioFuncionario (nome, sobrenome, email, senha, cargo, status) VALUES (:nome, :sobrenome, :email, :senha, :cargo, 1)");
      $pdo_conn->bindParam(':nome', $nome, PDO::PARAM_STR);
      $pdo_conn->bindParam(':sobrenome', $sobrenome, PDO::PARAM_STR);
      $pdo_conn->bindParam(':email', $email, PDO::PARAM_STR);
      $pdo_conn->bindParam(':senha', $senha, PDO::PARAM_STR);
